...
export default function Footer() {
  return (
    <footer className="bg-gray-100 py-12 pb-6 text-sm ">
      <div className="container mx-auto px-10 grid md:grid-cols-4 gap-8">
        <div>
          <h3 className="text-md font-bold mb-4 text-gray-800">TokoKu</h3>
          ...
          <ul className="space-y-2">
            {FOOTER_LINKS.produk.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="text-gray-600 hover:text-[#859F3D] transition-colors">
                  {link.label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t mt-8 pt-6 text-center">
        <p className="text-gray-600 text-sm">
          © {new Date().getFullYear()} TokoKu. Hak Cipta Dilindungi
          Undang-Undang.
        </p>
      </div>
    </footer>
